...
function ObraPrincipal({ idioma }) {
  ...
  const [mostrarResumoCompleto, setMostrarResumoCompleto] = useState(false);
  ...
  useEffect(() => {
    async function carregarDados() {
      try {
        ...
        //personagem
        const personagemData = await buscarPersonagem();
    ...
    <main className={styles.obraPrincipal}>
      ...
      <section className={styles.resumoArea}>
        ...
        ? (
            mostrarResumoCompleto
                ...
                : (idioma === 'pt'
                    ? livro.enredo_pt?.slice(0, 250)
                    : livro.enredo_en?.slice(0, 250)
                  ) + "..."
          )
        : "Carregando enredo..."}
          </p>
          ...
          <button
            className={styles.btnLerMais}
            onClick={() => setMostrarResumoCompleto(!mostrarResumoCompleto)}>
                {
                    mostrarResumoCompleto
                    ? (idioma === "pt" ? "Ler menos" : "Read less")
                    : (idioma == "pt" ? "Ler mais" : "Read more")
                }
            </button>
      ...
      <section className={styles.contextoSection}>
        ...
        <div className={styles.anoCard}>
          ...
          <span>
            {idioma === "pt" ? "ANO DE PUBLICAÇÃO" : "PUBLICATION YEAR"}
          </span>
        </div>
      </section>
      ...
      <section className={styles.conclusaoSection}>
        <h2 className={styles.conclusaoTitulo}>{idioma === "pt" ? "Conclusão" : "Conclusion"}</h2>

        <p className={styles.conclusaoSubtitulo}>
          {idioma === "pt"
            ? "Reflexões finais sobre a obra"
            : "Final reflections on the book"}
        </p>

        <p className={styles.conclusaoTexto}>
          {idioma === "pt" ? livro?.conclusao_pt : livro?.conclusao_en}
        </p>
      </section>
    </main>
  );
